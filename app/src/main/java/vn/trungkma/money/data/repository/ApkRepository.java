...
public class ApkRepository {
    ...
    @Inject
    public ApkRepository() {
    }

    public Single<List<FileData>> getPhoto() {
        return Single.fromCallable(() -> ApkRepository.this.getApkOnDevice()).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread());
    }

    private List<FileData> getApkOnDevice() {
        ...
        if (listFile != null && listFile.length > 0) {
            for (File file : listFile) {
                if (file.isDirectory()) {
                    ...
                    if (file.getName().endsWith(TYPE_APK)) {
