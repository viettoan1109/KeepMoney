...
import static vn.trungkma.money.common.Constant.ARCHIVE_BIN;
import static vn.trungkma.money.common.Constant.ARCHIVE_CAB;
import static vn.trungkma.money.common.Constant.ARCHIVE_GZIP;
import static vn.trungkma.money.common.Constant.ARCHIVE_JAR;
...
public class ArchiveRepository {
    ...
    public Single<List<FileData>> getArchive() {
        return Single.fromCallable(() -> ArchiveRepository.this.getArchiveOnDevice()).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread());
    }

    private List<FileData> getArchiveOnDevice(){
        ...
        if (listFile != null && listFile.length > 0) {
            for (File file : listFile) {
                if (file.isDirectory()) {
                    ...
                    if (file.getName().endsWith(ARCHIVE_7ZIP)
                            || file.getName().endsWith(ARCHIVE_BIN)
                            || file.getName().endsWith(ARCHIVE_CAB)
                            || file.getName().endsWith(ARCHIVE_RAR)
                            || file.getName().endsWith(ARCHIVE_GZIP)
                            || file.getName().endsWith(ARCHIVE_ZIP)
                            || file.getName().endsWith(ARCHIVE_JAR)
                            || file.getName().endsWith(ARCHIVE_TAR)) {
